...
class MainApp(App):
    ...
    def update_gas(self, *args):
        ...
        request_data = requests.get(f'https://api.etherscan.io/api?module=gastracker&action=gasoracle&apikey={defipulse_credentials.defipulseApikey}')
        data = request_data.json()

        # Create variables in gwei from api data (json)
        fast_gwei = data['FastGasPrice'] / 10
        average_gwei = data['ProposeGasPrice'] / 10
        slow_gwei = data['SafeGasPrice'] / 10
